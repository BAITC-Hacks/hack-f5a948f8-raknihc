...
import { ArrowRight, ArrowUpRight, BookOpen, Compass, Eye, EyeOff, History, Layers3, LayoutDashboard, LoaderCircle, LockKeyhole, LogOut, Menu, RefreshCw, Route, Sprout, ShieldCheck, X } from 'lucide-react'
...
import { HRWorkspace } from './HR'
import { RecommendationsView } from './Recommendations'

export type Tab = 'overview' | 'skills' | 'history' | 'trajectory' | 'recommendations'
const tabs = [
  ...
  { id: 'recommendations', label: 'Рекомендации', icon: Sprout },
] as const
...
export default function App() {
  ...
  useEffect(() => {
    if (!session?.user.employee_id || session.user.role === 'hr') return
  ...
  if (session.user.role === 'hr') return <HRWorkspace session={session} onLogout={logout} loggingOut={loggingOut} onUnauthorized={clearSession} />
    ...
    <div className="workspace"><header className="topbar"><div className="breadcrumb"><button className="icon-button mobile-menu" aria-label="Открыть меню" onClick={() => setMobileMenu(true)}><Menu size={21} /></button><span>Личный кабинет</span><span className="breadcrumb-slash">/</span><strong>{tabs.find(item => item.id === tab)?.label}</strong></div><div className="topbar-right"><span className="private-label"><ShieldCheck size={15} />Личный доступ</span><span className="avatar small">{initials(profile?.full_name ?? session.user.username)}</span></div></header>
      <main id="main-content" className="main-content">
        {!session.user.employee_id ? <section className="empty-state large"><span className="small-icon"><ShieldCheck size={26} /></span><h1>У этой учётной записи нет личного профиля</h1><p>Вы вошли как HR. Кабинет показывает развитие конкретного сотрудника. Для просмотра своих навыков войдите под учётной записью сотрудника или попросите администратора привязать ваш профиль.</p><button className="button primary" onClick={logout} disabled={loggingOut}>Выйти и сменить аккаунт<ArrowRight size={17} /></button></section> : <>
          <div className="page-heading"><div><span className="eyebrow">CAREER QUEST</span><h1>{tabs.find(item => item.id === tab)?.label}</h1><p>{tab === 'overview' ? 'Всё, что нужно для следующего шага в карьере.' : tab === 'skills' ? 'Ваши сильные стороны и пространство для роста.' : tab === 'history' ? 'Опыт, который становится частью вашего развития.' : tab === 'recommendations' ? 'Небольшой шаг сегодня. Новые возможности завтра.' : 'От сегодняшних навыков — к вашей карьерной цели.'}</p></div><button className="button refresh-button" onClick={() => setRevision(v => v + 1)} disabled={busy} aria-label="Обновить данные"><RefreshCw size={16} className={busy ? 'spin' : ''} /><span>Обновить</span></button></div>
          ...
          {data && <>{tab === 'overview' && <Overview data={data} navigate={navigate} />}{tab === 'skills' && <SkillsView data={data} />}{tab === 'history' && <HistoryView data={data} />}{tab === 'trajectory' && <TrajectoryView data={data} navigate={navigate} />}{tab === 'recommendations' && <RecommendationsView employeeId={data.profile.employee_id} token={session.access_token} refreshKey={revision} onUnauthorized={clearSession} onCompleted={() => setRevision(value => value + 1)} />}<footer className="page-footer"><span>Career Quest · Ваш маршрут развития</span><span>Срез данных: {dateLabel(data.trajectory.as_of_date)}</span></footer></>}
        </>}
      </main>
    </div>
  </div>
